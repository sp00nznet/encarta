...
def main():
    ...
    if is32:
        ...
        for start, body in funcs:
            ...
            # lift16 names a near-call target `res_<n>`, and n is NOT the
            # target: it is the lifter's base plus the target, a leftover from
            # flat DOS images. A call to offset 0 from a function based at
            # 0x00E4 is named res_0000E4, so reading the name as an offset
            # sends it to 0x00E4 - which in that case is the calling function
            # itself, and the recursion runs the host stack out.
            #
            # That was not a link error or a crash at the call site. It linked,
            # ran, and called the wrong function, which is the failure worth
            # going out of the way to avoid. The emitted comment carries the
            # decoded target, so use that and let the name be a label.
            c = near_call.sub(
                lambda m: "ir32_s%d_%04X(cpu);%s/* call 0x%s */"
                          % (a.seg, int(m.group(2), 16), m.group(1), m.group(2)), c)
            # A tail jump is NEAR, and lift16 turns it into a far dispatch by
            # splitting the target as a real-mode paragraph:
            #
            #   { recomp_dispatch(cpu, 0x5, 0xE); return; } /* tail-jcc 0x00005E */
            #
            # 0x005E is an offset in this segment, not 0005:000E. Left alone it
            # calls into some other segment entirely, and because that other
            # segment can jump back the same way, the result is unbounded
            # recursion - lifted calls are real C calls, so a cycle exhausts the
            # host stack rather than looping. DRV_LOAD died that way.
            #
            # The comment carries the true target, so use it.
            c = tail_jmp.sub(
                lambda m: "ir32_s%d_%04X(cpu); return;" % (a.seg, int(m.group(1), 16)), c)
            # A tail jump is compiled as a call, so a loop that spans two carved
            # functions becomes recursion rather than iteration - and a genuine
