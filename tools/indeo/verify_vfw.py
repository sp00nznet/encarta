"""Check the codec through Video for Windows, the way an application reaches it.

verify_rgb.py drives the recompiled driver directly. This drives nothing: it
installs the codec with ICInstall and then asks VFW to find a decompressor for
`IV32`, so every call goes out through msvfw32 and comes back in through the
registered DriverProc. If this passes, any caller that goes through ICM works -
which is what MCIAVI does on Encarta's behalf when the app calls mciSendCommand
and knows nothing about codecs at all.

    python verify_vfw.py [--limit N]
"""
import argparse
import glob
import os
...
RUN = os.path.join(HERE, "runtime", "build", "ir32_vfwtest.exe")
...
def main():
    ...
    if a.limit:
        files = files[:a.limit]
    tmp = tempfile.mkdtemp(prefix="ir32vfw")
    good = total = 0
    for path in files:
        out = subprocess.run(
        ...
        if os.path.exists(ppm):
            os.remove(ppm)
        r = subprocess.run([RUN, DLL, fr, str(W), str(H), ppm, "24"],
                           capture_output=True, text=True)
        total += 1
        if not os.path.exists(ppm):
            tail = [l for l in r.stdout.splitlines() if l.strip()][-1:] or ["no output"]
            print("%-16s %s" % (os.path.basename(path), tail[0]))
            continue
    ...
    print("\n%d of %d frames decode through Video for Windows" % (good, total))
